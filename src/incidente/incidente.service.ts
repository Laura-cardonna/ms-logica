...
export class IncidenteService {
  ...
  async obtenerHistorialPorBus(
    ...
    const query = this.incidenteBusRepository.createQueryBuilder('incidenteBus')
  ...
  async actualizarSeguimiento(
    id: number,
    datos: { estado?: 'pendiente' | 'en_revision' | 'resuelto'; comentario?: string; adminNombre: string }
  ) {
    ...
    if (!incidenteBus) {
      throw new NotFoundException('No se encontró el reporte técnico del bus solicitado.');
    }
    ...
    if (datos.comentario) {
      ...
      const comentariosActuales = Array.isArray(incidente.comentarios) ? incidente.comentarios : [];
  ...
  async getTendenciaIncidentes(filtro: { empresaId?: number }) {
    ...
    const query = this.incidenteBusRepository.createQueryBuilder('incidenteBus')
      ...
      .select("DATE_FORMAT(COALESCE(incidente.fecha, incidenteBus.timestamp, CURRENT_TIMESTAMP), '%Y-%m')", 'mes') // Eje X agrupado por Año-Mes para MySQL
      .addSelect('incidenteBus.tipo', 'tipo') // Eje Y / Líneas diferenciadas por Tipo
      .addSelect('COUNT(incidenteBus.id)', 'cantidad')
      .where('(incidente.fecha >= :fechaLimite OR incidenteBus.timestamp >= :fechaLimite)', { fechaLimite })
      .groupBy("DATE_FORMAT(COALESCE(incidente.fecha, incidenteBus.timestamp, CURRENT_TIMESTAMP), '%Y-%m')")
      .addGroupBy('incidenteBus.tipo')
      .orderBy('mes', 'ASC');

    // 🏢 Si se selecciona una empresa en la UI, filtramos usando el ID de la tabla Bus
    if (filtro.empresaId) {
      query.andWhere('bus.empresaId = :empresaId', { empresaId: filtro.empresaId });
    }
